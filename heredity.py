...
# This is done by Abdullah: Heredity
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if people[person]["mother"] is None and people[person]["father"] is None:
            ...
            # Person has parents, so calculate based on parents' genes
            ...
            # Calculate probability of inheriting genes from each parent
            prob_mother_passes_gene = 0
            prob_father_passes_gene = 0

            # Probability mother passes the gene
            if mother_genes == 0:
                prob_mother_passes_gene = PROBS["mutation"]
            elif mother_genes == 1:
                prob_mother_passes_gene = 0.5
            elif mother_genes == 2:
                prob_mother_passes_gene = 1 - PROBS["mutation"]

            # Probability father passes the gene
            if father_genes == 0:
                prob_father_passes_gene = PROBS["mutation"]
            elif father_genes == 1:
                prob_father_passes_gene = 0.5
            elif father_genes == 2:
                prob_father_passes_gene = 1 - PROBS["mutation"]

            # Calculate probability based on number of genes child has
            if num_genes == 0:
                # Child gets gene from neither parent
                probability *= (1 - prob_mother_passes_gene) * (1 - prob_father_passes_gene)
            elif num_genes == 1:
                # Child gets gene from exactly one parent
                probability *= (prob_mother_passes_gene * (1 - prob_father_passes_gene) +
                               (1 - prob_mother_passes_gene) * prob_father_passes_gene)
            elif num_genes == 2:
                # Child gets gene from both parents
                probability *= prob_mother_passes_gene * prob_father_passes_gene

        # Multiply by probability of trait
